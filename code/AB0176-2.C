...
extern byte r_num,g_num,b_num,step,r_maxnum,g_maxnum,b_maxnum;
extern byte pwm_time2,pwm_time,pwm_time3,led_mode_temp;
void data_init(void)
{
	f_work=0;	
	f_key_scan=0; 
...
void	Interrupt (void)
{
	...
	{	
		Intrq.T16	=	0;
		if(pwm_time2<250)pwm_time2++;
		if(pwm_time3<250)pwm_time3++;
		if(pwm_time<250)pwm_time++;
		if (--count == 0)					
		{
